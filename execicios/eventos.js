...
bot.on('text', ctx => {
    ctx.reply(`Texto ${ctx.update.message.text} recebida com sucesso`);
});
...
bot.on('contact', ctx => {
    ...
    console.log(contact);
    ctx.reply(`Vou lembra de ${contact.first_name} (${contact.phone_number})`)
});
...
bot.on('sticker', ctx => {
    var sticker = ctx.update.message.sticker;
    console.log(sticker);
