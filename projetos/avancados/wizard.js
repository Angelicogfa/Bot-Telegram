...
precoHandler.on('message', ctx => ctx.reply('Apenas números são aceitos'));

const dataHandler = new Composer()
dataHandler.hears(/(\d{2}\/\d{2}\/\d{4})/, ctx => {
    data = ctx.match[1];
    console.log(ctx.match);
    ctx.reply(`Aqui está a descrição da sua compra:
        Descrição: ${descricao}
        Preço: ${preco}
        Data: ${data}.
...
dataHandler.use(ctx => ctx.reply('Entre com uma data válida no formato dd/MM/yyyy'));
...
confirmacaoHandler.on('message', ctx => ctx.reply('Apenas confirme', confirmacao));
